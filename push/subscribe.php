<?php
// Abo-Endpoint: speichert/entfernt ein Push-Abo des Browsers.
...
if ($action === 'subscribe') {
    ...
    if ($endpoint === '' || $p256dh === '' || $authKey === '') {
        http_response_code(400);
        echo json_encode(['ok' => false, 'error' => 'Abo unvollständig.']);
        exit;
    }

    $ua = substr((string) ($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255);

    // Kategorie-Präferenz (Safety immer implizit). Fehlt das Feld (alter Client),
    // bleibt die Spalte NULL = „alle". Leeres Array = nur Safety.
    ...
    if (array_key_exists('categories', $body)) {
        ...
        $categories = implode(',', $sel); // '' möglich (nur Safety)
    }

    // „Mein Plan": favorisierte Slot-IDs. Fehlt das Feld (alter Client) → NULL
    // (unverändert lassen). Leeres Array = kein Plan-Abo.
    $plan = null;
    ...
    $stmt = $pdo->prepare(
        'INSERT INTO push_subscriptions (endpoint, p256dh, auth, ua, categories, plan)
         VALUES (?, ?, ?, ?, ?, ?)
    ...
    $stmt->execute([$endpoint, $p256dh, $authKey, $ua, $categories, $plan]);
    // Protokoll ohne Abo-Details (Endpoint wäre pseudo-identifizierend).
    require_once __DIR__ . '/log.php';
    $total = (int) $pdo->query('SELECT COUNT(*) FROM push_subscriptions')->fetchColumn();
    app_log('info', 'subscribe', "Abo gespeichert/aktualisiert ($total Abos gesamt)");
...
if ($action === 'unsubscribe') {
    ...
    if ($endpoint !== '') {
        ...
        require_once __DIR__ . '/log.php';
        $total = (int) $pdo->query('SELECT COUNT(*) FROM push_subscriptions')->fetchColumn();
        app_log('info', 'subscribe', "Abo entfernt ($total Abos gesamt)");
    }
    echo json_encode(['ok' => true]);
    exit;
}
