...
**🇩🇪 [Deutsch](TELEGRAM.md) · 🇫🇷 [Français](TELEGRAM.fr.md) · 🇪🇸 [Español](TELEGRAM.es.md)**
...
- **Tags** (removed from the text) control options:
  ...
  - `#push` → additionally send as **web push** to the lock screen. **`#safety`** pushes **automatically** (even without `#push`). Push only for **immediate** publication (not for `@HH:mm`-scheduled ones) and only if web push is set up (`docs/PUSH.en.md`). Controllable via `pushAutoCategories` in `config.php`.
- **Commands:**
