...
// das PHP-Backend (/push/subscribe.php). Der VAPID-Public-Key kommt aus der
// Build-Env (VITE_VAPID_PUBLIC_KEY); ohne Key ist Push deaktiviert.
...
/** Ist Web-Push in diesem Browser grundsätzlich möglich und konfiguriert? */
...
export async function subscribePush(): Promise<PushSubscription> {
  if (!VAPID_PUBLIC_KEY) throw new Error("Kein VAPID-Public-Key konfiguriert.");
